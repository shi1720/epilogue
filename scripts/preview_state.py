"""Populate a database with a representative mid-case state — UI preview only.

This exists so designers/reviewers can see the dashboard without model
credentials. It writes ledger records directly; no agent is involved and no
model output is imitated beyond shape. The real thing: `epilogue demo` with
credentials, which runs the full agent pipeline live.
...
print("Run: EPILOGUE_DATA_DIR=data-preview epilogue serve")
