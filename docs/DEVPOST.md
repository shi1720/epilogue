...
- **Model-agnostic** via Strands' provider abstraction: Amazon Bedrock (Claude Sonnet) by default; Anthropic, Gemini (free-tier key — judges can run the live demo at zero cost), OpenAI, or Ollama with one env var.
