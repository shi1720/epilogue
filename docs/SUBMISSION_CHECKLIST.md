...
- [ ] **Live demo link** (projects with one score higher): easiest is `./deploy.sh` (Cloud Run + optional *.web.app URL — see docs/DEPLOY_FIREBASE.md). AWS-native alternative: App Runner straight from the Dockerfile — push the image to ECR (`docker build -t epilogue . && aws ecr …`), create an App Runner service (0.25 vCPU is plenty), set the AWS credentials/region env vars, and paste the service URL into Devpost. Your $50 AWS credits cover this for weeks. (Add HTTP basic auth or note it's a shared demo — one case at a time.)
